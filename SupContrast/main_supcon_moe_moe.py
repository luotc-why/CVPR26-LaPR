...
from dataset_tmoe import contrastive_dataset
import numpy as np
...
def parse_option():
    ...
    opt.model_path = f'{opt.data_base_path}/save/final_seg_SupCon_moe_moe/{opt.dataset}_moe_moe_models'
    opt.tb_path = f'{opt.data_base_path}/save/final_seg_SupCon_moe_moe/{opt.dataset}_moe_moe_tensorboard'
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        ann_root = os.path.join(opt.data_base_path, 'figures_dataset/pascal-5i/VOC2012/SegmentationClassAug')
        train_dataset = contrastive_dataset(data_base_path=opt.data_base_path,img_root=opt.data_folder, ann_root=ann_root, meta=opt.meta, folder_id=opt.folder_id,
