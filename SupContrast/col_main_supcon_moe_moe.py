...
from dataset_tmoe_col import contrastive_dataset
import numpy as np
...
def parse_option():
    ...
    opt.model_path = f'{opt.data_base_path}/save/final_col_SupCon_moe_moe/{opt.dataset}_moe_moe_models'
    opt.tb_path = f'{opt.data_base_path}/save/final_col_SupCon_moe_moe/{opt.dataset}_moe_moe_tensorboard'
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        ann_root = os.path.join(opt.data_base_path, 'figures_dataset/imagenet/train_label')
        train_dataset = contrastive_dataset(data_base_path=opt.data_base_path,img_root=opt.data_folder, ann_root=ann_root, folder_id=opt.folder_id,
                                            transform=train_transform)
    else:
        raise ValueError(opt.dataset)

    train_sampler = RandomSubsetSampler(train_dataset, 1000)
