...
def get_args():
    ...
    parser.add_argument('--top_50_path', default='/data1/liyusheng/CVPR-LRKL/Data/figures_dataset/imagenet/final_moe_features_vit-laion2b_col_lr_0.01_epoch_ckpt_epoch_200/new_top50-similarity.json')
    ...
    parser.add_argument('--part', type=int, default=0, help='index of this part, from 0 to 4')
...
def evaluate(args):
    with open(os.path.join(args.output_dir, f'log.txt'), 'w') as log:
        log.write(str(args) + '\n')
    ...
    mask_transform = torchvision.transforms.Compose(
        [torchvision.transforms.Resize((224 // 2 - padding, 224 // 2 - padding), 3),
         convert_to_rgb,
         torchvision.transforms.ToTensor()])
    ds = DatasetColorization(args.data_path, image_transform, mask_transform, top_50_path=args.top_50_path, split = args.meta_split, feature_name=args.feature_name, seed=args.seed)
    dataloaders = DataLoader(ds, batch_size=50, shuffle=False,num_workers=4)
    image_number = 0
    ...
    for data in tqdm(dataloaders):
        grid = data['grids']  
        grid = grid[:,0,:,:,:]
        grid = (grid - imagenet_mean[None, :, None, None]) / imagenet_std[None, :, None, None]

        originals_b, gens_b = _generate_result_for_canvas_batch(args, model, grid)

        for idx in range(len(originals_b)):
            original_image = originals_b[idx]
            generated_result = gens_b[idx]
            # print(f'Processing image {idx}, sim idx {sim_idx}')
            current_metric = calculate_metric(args, original_image, generated_result)
            with open(os.path.join(args.output_dir, f'log.txt'), 'a') as log:
                log.write(f'{image_number}\t0\t{current_metric}\n')
            image_number += 1
