...
def get_args():
    ...
    parser.add_argument('--part', type=int, default=1, help='index of this part, from 0 to 9')
    parser.add_argument('--num_parts', type=int, default=10, help='total number of parts')
...
@torch.no_grad()
def calculate_metric(args, ours):
    ours = ours.to(torch.float32)
    if ours.max() > 1.0:
        ours = ours / 255.0
    mean = torch.tensor(imagenet_mean, device=ours.device).view(3, 1, 1)
    std  = torch.tensor(imagenet_std, device=ours.device).view(3, 1, 1)
    ours = (ours - mean) / std

    target = ours[:, :111, 113:]
    ours_cropped = ours[:, 113:, 113:]
    diff = target - ours_cropped
...
def evaluate(args):
    log_path = os.path.join(args.output_dir, f'log_part{args.part}.txt')
    ...
    ds = DatasetColorization(
        args.data_path, image_transform, mask_transform,
        split=args.meta_split, feature_name=args.feature_name, seed=args.seed
    )
    ...
    for idx in trange(start_idx, end_idx):
        ...
        for start in range(0, SIM_TOTAL, B):
            ...
            canvases_b = torch.stack([grid[s] for s in chunk_idx], dim=0).to(args.device, non_blocking=True)

            ours_b = canvases_b.to(torch.float32)
            if ours_b.max() > 1.0:
                ours_b = ours_b / 255.0
            ours_b = (ours_b - mean) / std

            target = ours_b[:, :, :111, 113:]
            ours_crop = ours_b[:, :, 113:, 113:]

            mse_b = (target - ours_crop).pow(2).mean(dim=(1, 2, 3))

            for local_i, sim_idx in enumerate(chunk_idx):
                mse_val = float(mse_b[local_i])
                results_cache.append(f'{idx}\t{sim_idx}\t{{"mse": {mse_val:.6f}}}')
                eval_dict['mse'] += (mse_val / len(ds))

        if (idx + 1) % 5 == 0 or (idx + 1) == len(ds):
