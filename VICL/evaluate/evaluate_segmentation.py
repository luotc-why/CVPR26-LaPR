...
def get_args():
    ...
    parser.add_argument('--top_50_path', default='/data1/liyusheng/CVPR-LRKL/Data/figures_dataset/pascal-5i/VOC2012/final_moe_features_vit-laion2b_det_lr_0.005_epoch_ckpt_epoch_200/LaPR_top50-similarity.json')
...
def evaluate(args):
    ...
    #                      flipped_order=args.flip, purple=args.purple, random=args.random, cluster=args.cluster, feature_name=args.feature_name,top_50_path = args.top_50_path, percentage=args.percentage, seed=args.seed)
    ds = CanvasDataset(top_50_path=args.top_50_path)
    model = prepare_model(args.ckpt, arch=args.model, vq_base_dir=args.base_dir)
    ...
    for idx in trange(len(ds)):
        ...
        for grid_idx, canvas in enumerate(canvas_stack):
            ...
            if args.output_dir:
                ...
                if grid_idx in [0,20,40]:
                    Image.fromarray(np.uint8(generated_result)).save(
                        os.path.join(args.output_dir, f'generated_{idx}_{grid_idx}.png'))
            if args.purple:
            ...
            with open(os.path.join(args.output_dir, 'log.txt'), 'a') as log:
                ...
                log.write(str(idx) + '\t' + str(grid_idx) + '\t' + str(current_metric) + '\n')
