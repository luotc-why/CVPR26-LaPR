...
def prepare_model(chkpt_dir, arch='mae_vit_large_patch16', vq_base_dir='.', device='cpu'):
    # build model
    model = getattr(models_mae, arch)(vq_base_dir)
    # load model
...
@torch.no_grad()
def generate_image_for_batch(orig_image, model, ids_shuffle, len_keep: int, device: str = 'cpu'):
    """ids_shuffle is [bs, 196]"""
    # print(orig_image.shape)
    mask, orig_image, x = generate_raw_prediction_for_batch(device, ids_shuffle, len_keep, model, orig_image)
    num_patches = 14
    # print(x.shape)
    y = x.argmax(dim=-1)
    # print(y.shape)
    # assert False
    im_paste, mask, orig_image = decode_raw_predicion_for_batch(mask, model, num_patches, orig_image, y)
    # print(im_paste.shape, 'im_paste.shape in generate_image_for_batch')
    # assert False
    return orig_image, im_paste, mask



def decode_raw_predicion_for_batch(mask, model, num_patches, orig_image, y):
    # print(y.shape, 'y.shape in decode_raw_predicion_for_batch')
    ...
    # print(y.shape, 'y.shape after decode in decode_raw_predicion_for_batch')
    y = F.interpolate(y, size=(224, 224), mode='bilinear').permute(0, 2, 3, 1)
    y = torch.clip(y * 255, 0, 255).int().detach().cpu()
    # visualize the mask
    # print(y.shape, 'y.shape before mask in decode_raw_predicion_for_batch')
    mask = mask.unsqueeze(-1).repeat(1, 1, model.patch_embed.patch_size[0] ** 2 * 3)
    # print(mask.shape, 'mask.shape before unpatchify in decode_raw_predicion_for_batch')
    mask = model.unpatchify(mask)  # 1 is removing, 0 is keeping
    # print(mask.shape, 'mask.shape after unpatchify in decode_raw_predicion_for_batch')
    mask = torch.einsum('nchw->nhwc', mask).detach().cpu()
    # print(mask.shape, 'mask.shape after einsum in decode_raw_predicion_for_batch')
    orig_image = torch.einsum('nchw->nhwc', orig_image)
    orig_image = (
        torch.clip((orig_image.cpu().detach() * imagenet_std + imagenet_mean) * 255, 0, 255).int())
    # MAE reconstruction pasted with visible patches
    # print(orig_image.shape, 'orig_image.shape before im_paste in decode_raw_predicion_for_batch')
    # print(mask.shape, 'mask.shape before im_paste in decode_raw_predicion_for_batch')
    # print(y.shape, 'y.shape before im_paste in decode_raw_predicion_for_batch')
...
@torch.no_grad()
def generate_raw_prediction_for_batch(device, ids_shuffle, len_keep, model, orig_image):
