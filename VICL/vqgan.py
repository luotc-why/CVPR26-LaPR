...
class VectorQuantizer2(nn.Module):
    ...
    def get_codebook_entry(self, indices, shape):
        ...
        # print(z_q.shape, 'zqshape')
        if shape is not None:
            z_q = z_q.view(shape)
            # reshape back to match original input shape
            z_q = z_q.permute(0, 3, 1, 2).contiguous()
        # print(z_q.shape, 'finalshape')
        return z_q
...
def get_vq_model(vq_ckpt_dir='.'):
    config = OmegaConf.load(os.path.join(vq_ckpt_dir, 'weights/vqgan/model.yaml'))
    model = VQModel(**config.model.params)
    sd = torch.load(os.path.join(vq_ckpt_dir, "weights/vqgan/last.ckpt"), map_location="cpu")["state_dict"]
